...
- `yarn start` / `yarn web` — Expo dev server only, without functions/emulators.
- `yarn ios` / `yarn android` — `expo run:*`; builds and installs a **dev build**. The app uses native modules (Sentry, Reanimated, SVG, Apple auth), so Expo Go is not an option.
...
Requires Xcode + CocoaPods (`brew install cocoapods`). `ios/` is generated by CNG and is gitignored — never commit it.
...
- **`SENTRY_DISABLE_AUTO_UPLOAD=true` is required locally.** The `@sentry/react-native` build phase otherwise tries to upload source maps and fails the build with `error: An organization ID or slug is required`, because org/project aren't configured in `app.json`. The native compile succeeds — only the bundling script phase fails, which makes it look like a code error when it isn't.
- **Metro and the Firestore emulator both want a port.** Metro owns 8081 by default, so the Firestore emulator uses Firebase's default 8080 (`firebase.json` + `src/store/index.ts` must agree). Under the old SDK 37 setup Metro ran on 19000/19001 and Firestore sat on 8081; don't move it back.

If `expo run:ios` fails with `No code signing certificates are available`, it has picked a non-simulator destination. Build straight at the simulator instead:

```sh
xcodebuild -workspace ios/TurnipBuds.xcworkspace -scheme TurnipBuds \
